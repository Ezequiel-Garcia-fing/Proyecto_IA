...
def load_images_and_extract_features(folder_path, save_csv=False):
    data = {
        'filename': [], 'label': [],
        'hu_1': [], 'hu_2': [], 'hu_3': [], 'hu_4': [],
...
def recognize_images_pca(centroids, pca, label_map, show_plots=False):
    """
    ...
    for image_file in test_files:
        ...
        if None not in features:
            ...
            results.append({'filename': image_file, 'predicted_label': predicted_label})
...
# Cargar datos de entrenamiento y generar el archivo CSV
df_train = load_images_and_extract_features('base_datos_img', save_csv=False)

# Inicializar los centroides utilizando las etiquetas en el espacio PCA
initial_centroids, transformed_features, pca = initialize_centroids_pca_labels(df_train)
...
# Mostrar el gráfico con los centroides inicializados
plot_pca_3d(df_train, centroids=initial_centroids)
...
recognize_images_pca(centroids=centroids, pca=pca, label_map=label_map, show_plots=False)
